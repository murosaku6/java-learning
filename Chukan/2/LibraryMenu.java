/**
 *  コンソール画面の操作を担当するクラス
 */
import java.util.Scanner;

public class LibraryMenu {
    /**
     * メインメニューを表示し、選択された番号を返す
     * 
     * @param scanner　キーボード入力
     * @return　入力されたメニュー番号
     */
    public static int showMainMenu(Scanner scanner){
        System.out.println();
        System.out.println("===== 図書館業務管理システム =====");
        System.out.println("1. 書籍登録");
        System.out.println("2. 書籍一覧");
        System.out.println("3. 書籍検索");
        System.out.println("4. 書籍削除");
        System.out.println("5. 貸出・返却");
        System.out.println("6. ジャンル管理");
        System.out.println("0. 終了");

        System.out.print("番号を入力してください：");

        return readInt(scanner, 0, 7);
    }

    /**
     * 書籍を登録する
     * 
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void registerBook(Scanner scanner, LibraryManager manager) {
        System.out.println();
        System.out.println("===== 書籍登録 =====");

        System.out.print("タイトル：");
        String title = scanner.nextLine();

        System.out.print("著者：");
        String author = scanner.nextLine();

        Book book = new Book(title, author);

        manager.addBook(book);
    }

    /**
     * 書籍一覧の表示方法を選択する
     * 
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void showBooksMenu(Scanner scanner, LibraryManager manager) {
        while (true) {
            System.out.println();
            System.out.println("===== 書籍一覧 =====");
            System.out.println("1. 登録順");
            System.out.println("2. タイトル順（昇順）");
            System.out.println("3. タイトル順（降順）");
            System.out.println("4. 貸出状況順");
            System.out.println("0. 戻る");
            System.out.print("選択：");

            int choice = readInt(scanner,0,4);

            switch (choice) {
                case 1:
                    manager.showBooks(LibraryManager.ORDER_REGISTER);
                    break;

                case 2:
                    manager.showBooks(LibraryManager.ORDER_TITLE_ASC);
                    break;

                case 3:
                    manager.showBooks(LibraryManager.ORDER_TITLE_DESC);
                    break;

                case 4:
                    manager.showBooks(LibraryManager.ORDER_BORROW);
                    break;

                case 0:
                    return;

                default:
                    System.out.println("0～4を入力してください。");
            }
        }
    }

    /**
     * 書籍検索メニューを表示する
     * 
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void showSearchMenu(Scanner scanner, LibraryManager manager) {
        while (true) {
            System.out.println();
            System.out.println("===== 書籍一覧 =====");
            System.out.println("1. タイトル検索");
            System.out.println("2. 著者検索");
            System.out.println("3. ジャンル検索");
            System.out.println("4. Id検索");
            System.out.println("0. 戻る");
            System.out.print("選択：");

            int choice = readInt(scanner,0,4);

            switch (choice) {
                case 1:
                    System.out.print("タイトル：");
                    manager.searchByTitle(scanner.nextLine());
                    break;

                case 2:
                    System.out.print("著者：");
                    manager.searchByAuthor(scanner.nextLine());
                    break;

                case 3:
                    System.out.print("ジャンル：");
                    manager.searchByGenre(scanner.nextLine());
                    break;

                case 4:
                    System.out.print("Id：");
                    manager.searchById(choice);
                    break;

                case 0:
                    return;

                default:
                    System.out.println("0～4を入力してください。");
            }
        }
    }

    /**
     * 書籍を削除する
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void deleteBook(Scanner scanner, LibraryManager manager) {
        System.out.println();
        System.out.println("===== 書籍削除 =====");
        System.out.print("削除する管理ID：");
        int id = readInt(scanner);
        manager.deleteBook(id);
    }

    /**
     * 貸出・返却メニューを表示する
     * 
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void showBorrowMenu(Scanner scanner, LibraryManager manager){
        while(true){
            System.out.println();
            System.out.println("===== 書籍・返却 =====");
            System.out.println("1. 貸出");
            System.out.println("2. 返却");
            System.out.println("3. 貸出・返却履歴");
            System.out.println("0. 戻る");
            System.out.print("選択：");

            int choice = readInt(scanner, 0, 3);

            switch(choice){
                case 1:{
                    manager.showBooks(LibraryManager.ORDER_REGISTER);
                    System.out.print("ID：");
                    int id = readInt(scanner);
                    System.out.print("利用者名：");
                    String borrower = scanner.nextLine();
                    manager.borrowBook(id, borrower);
                    break;
                }

                case 2:{
                    manager.showBooks(LibraryManager.ORDER_REGISTER);
                    System.out.print("Id：");
                    int id = readInt(scanner);
                    manager.returnBook(id);
                    break;
                }
                
                case 3:{
                    System.out.println();
                    System.out.println("===== 貸出・返却履歴 =====");
                    manager.showBorrowHistory();
                    break;
                }

                case 0:
                    return;

                default:
                    System.out.println("0～3を入力してください。");
            }
        }
    }

    /**
     * ジャンル管理メニューを表示する
     * 
     * @param scanner　キーボード入力
     * @param manager　図書館管理クラス
     */
    public static void showGenreMenu(Scanner scanner, LibraryManager manager){
        while(true){
            System.out.println();
            System.out.println("===== ジャンル管理 =====");
            System.out.println("1. ジャンル追加");
            System.out.println("2. ジャンル削除");
            System.out.println("3. ジャンル一覧表示");
            System.out.println("0. 戻る");
            System.out.print("選択：");

            int choice = readInt(scanner,0,3);

            switch(choice){
                case 1:
                    System.out.print("Id：");
                    int id = readInt(scanner);
                    System.out.print("追加するジャンル：");
                    String genre = scanner.nextLine();
                    manager.addGenre(id, genre);
                    break;

                case 2:
                    System.out.print("Id：");
                    id = readInt(scanner);
                    System.out.print("削除するジャンル：");
                    genre = scanner.nextLine();
                    manager.removeGenre(id, genre);
                    break;

                case 3:
                    System.out.print("Id：");
                    id = readInt(scanner);
                    manager.showGenres(id);
                    break;

                case 0:
                    return;

                default:
                    System.out.println("0～3を入力してください。");
            }
        }
    }

    /**
     * 数値を入力する
     * 
     * @param scanner　キーボード入力
     * @return　入力された整数
     */
    private static int readInt(Scanner scanner) {
        while (true) {
            try {
                return Integer.parseInt(scanner.nextLine());
            } catch (NumberFormatException e) {
                System.out.print("数字を入力してください：");
            }
        }
    }

    /**
     * 指定した範囲の整数を入力する
     * 
     * @param scanner　キーボード入力
     * @param min　最小値
     * @param max　最大値
     * @return　指定範囲内の整数
     */
    private static int readInt(Scanner scanner, int min, int max) {
        while (true) {
            int value = readInt(scanner);
            if (value >= min && value <= max) {
                return value;
            }
            System.out.printf("%d～%dを入力してください：", min, max);
        }
    }
}
